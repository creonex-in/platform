...
export default async function LearnerLayout({ children }: { children: React.ReactNode }): Promise<React.ReactElement> {
  ...
  const isCreator = parseRoles(user.role).includes('creator')

  return (
    <SidebarProvider>
      <AppSidebar role="learner" userName={displayName} userInitials={getInitials(displayName)} isCreator={isCreator} />
