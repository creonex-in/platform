...
const STEPS: Step[] = [
  {
    ...
    bgColorClass: "bg-blue-50 border-blue-100 hover:border-blue-300 hover:bg-blue-100/70",
  },
  {
    id: "offering",
    number: "02",
    ...
    bgColorClass: "bg-violet-50 border-violet-100 hover:border-violet-300 hover:bg-violet-100/70",
  },
  {
    id: "split-payout",
    number: "03",
    ...
    bgColorClass: "bg-emerald-50 border-emerald-100 hover:border-emerald-300 hover:bg-emerald-100/70",
  },
];
    ...
    <section id="how-it-works" className="bg-gradient-to-b from-white to-slate-50 py-24 md:py-32 relative overflow-hidden border-t border-b border-slate-200">
      ...
      <div className="page-container relative z-10">
        ...
        <div className="mx-auto mb-16 flex max-w-3xl flex-col items-center text-center md:mb-20">
          ...
          <h2 className="text-4xl font-bold tracking-tight text-zinc-900 sm:text-5xl lg:text-[56px] lg:leading-[1.1] font-display">
            Simple steps to earning online.
          </h2>
          <p className="text-lg text-zinc-500 leading-relaxed font-light max-w-2xl mt-4 font-sans">
        ...
        <div className="w-full grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:gap-8 relative z-10">
          {STEPS.map((step) => (
            <div
              key={step.id}
              className={`group relative flex w-full flex-col items-center overflow-hidden rounded-[2.5rem] px-6 py-12 shadow-sm transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_12px_30px_rgba(30,41,59,0.12)] sm:px-8 lg:py-14 border ${step.bgColorClass}`}
            >
              {/* Step Number Badge */}
              <span className="absolute top-6 left-6 text-xs font-bold text-zinc-600 font-mono bg-white border border-slate-200 px-3 py-1 rounded-full shadow-sm">
                Step {step.number}
              </span>

              {/* Floating Image Composition */}
              <div className="relative aspect-[4/3] w-full max-w-[280px] overflow-hidden rounded-[20px] shadow-md border border-slate-200 transition-all duration-500 group-hover:scale-105 group-hover:shadow-lg bg-slate-100">
                <Image
                  src={step.imageSrc}
                  alt={step.imageAlt}
                  fill
              ...
              <div className="mt-10 flex flex-col items-center text-center">
                <h3 className="mb-3 text-2xl font-bold tracking-tight text-zinc-900 lg:text-3xl font-display">
                  {step.title}
                </h3>
                <p className="max-w-[300px] text-sm font-light leading-relaxed text-zinc-500 font-sans">
                  {step.description}
                </p>
              </div>
            </div>
